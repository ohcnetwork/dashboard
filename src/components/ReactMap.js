...
export default function MapContainer(){
    ...
    return (
      <div id="mapid" className="h-full">
        <Map center={center} zoom={12} minZoom={10}>
          ...
          />
            {facilityData.map(facility => {
              ...
              return (
                <SVGOverlay bounds={[[facility.lat, facility.lng+0.5],[facility.lat-0.25, facility.lng]]}>
        ...
        <div style={{position:"absolute", background:"white", top:"85px", right:"15px", padding:"20px", zIndex:"100"}}>
          <div style={{borderBottom:"2px solid #eaeaea"}}>
          	<ul className='flex cursor-pointer'>
                <li className={"py-2 px-6 bg-white rounded-t-lg" + (selected.name==="ventilator" ? "" : " text-gray-500 bg-gray-200")} onClick={_=>setSelected({name:"ventilator",current:"ventcurrent"})}>Ventilator</li>
                <li className={"py-2 px-6 bg-white rounded-t-lg" + (selected.name==="icu" ? "" : " text-gray-500 bg-gray-200")} onClick={_=>setSelected({name:"icu",current:"icucurrent"})}>ICU</li>
          	</ul>
          </div>
        </div>
      </div>
